...
<div class="container">  
    ...
    $id=$this->session->userdata('id');
     echo form_open("Update/edit_cowner/$id", array('id' => 'contact', 'enctype' => 'multipart/form-data')); ?>
    <h3>My Data</h3>
    <img class="im" src=<?php echo base_url(); ?>images/line.png>

    <fieldset>
       
        Name<input class="form-control" value=<?php foreach ($car_owner as $k) 
        {
            echo $k->username;
   ...
   <br> <div  style="" id="name_error"></div>    
    ...
    <fieldset>
        ...
        Email :<input         <input value=<?php foreach ($car_owner as $k) 
        {
            echo $k->email;
  ...
  <fieldset>
      Password<input placeholder="Password"          value=<?php foreach ($car_owner as $k) 
        {
            echo $k->password;
...
<!--
 ...
 <div  style="float: right;position: relative;right: 150px;top:0px;color: green;font-size: 20px;font-family: cursive" id="phone_error"></div>
    -->
  <fieldset>
        Age<input placeholder="Age "      <input value=<?php foreach ($car_owner as $k) 
        {
            echo $k->age;
